...
const db = {
  set(file, key, value) {
    const keyv = new Keyv(`sqlite://./db/${file}.sqlite`, { namespace: file });
...
client.on("messageCreate", async (message) => {
  ...
  if (message.guild.id === "1056948917633286214") {
    if (message.author.id === "1002151638028525599")
      return;
...
client.on("messageCreate", async (message) => {
  ...
  data.name = "Kuu";
  if (message.content === "t.1") {
    console.log(data);
  }
  if (message.content === "t.2") {
